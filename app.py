...
# --- CONFIGURATION ---
# This is the bridge to your Google Sheet
...
def amy_admin():
    ...
    for b in bookings_raw:
        # Mapping for YOUR Sheet: [0:Date, 1:Contact, 2:School, 3:Address, 4:Staff, 5:Hours, 6:Notes, 7:Status]
        if len(b) >= 3:
...
def teacher_order(delivery_date):
    ...
    menu_items = []
    try:
        response = requests.post(GOOGLE_SCRIPT_URL, json={"action": "get_menu", "sunday_anchor": anchor}, timeout=15)
    ...
    return render_template('orderform.html', delivery_date=delivery_date, deadline=deadline, menu=menu_items, school_name=school)
...
def submit_order():
    ...
    try:
        ...
        return "Order received, but failed to sync with Sheet."
