...
def get_sunday_anchor(delivery_date_str):
    try:
        ...
        # Skip if it's the header "Lunch Date"
...
def amy_admin():
    ...
    refined = []
    # Skip the header row [0] by starting at index 1
    for b in bookings_raw[1:]: 
        # Mapping: [0:Date, 1:Contact, 2:School, 3:Address, 4:Staff, 5:Hours, 6:Notes, 7:Status]
        try:
            if len(b) >= 3 and b[2]: # Ensure there is at least a school name
